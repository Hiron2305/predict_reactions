...
def evaluate_model(model, X_test_files, y_test, batch_size=1024):
    ...
    with torch.no_grad():
        ...
        for filename in X_test_files:
            ...
            # Получаем целевые значения, соответствующие размеру текущего батча
            end_idx = current_idx + X_test_tensor.shape[0]
            if end_idx > len(y_test):
                logging.warning(f"Skipping batch due to size mismatch: expected {X_test_tensor.shape[0]}, got {len(y_test) - current_idx}")
                break

            y_batch = y_test.iloc[current_idx:end_idx].values
            if len(y_batch) != X_test_tensor.shape[0]:
                logging.error(f"Skipping batch due to size mismatch: expected {X_test_tensor.shape[0]}, got {len(y_batch)}")
                continue

            all_targets.append(y_batch)
            current_idx = end_idx

        # Преобразуем списки предсказаний и целей в numpy массивы
        all_predictions = np.vstack(all_predictions)
        all_targets = np.vstack(all_targets)

        # Диагностика размеров
        logging.info(f"Final shapes - Predictions: {all_predictions.shape}, Targets: {all_targets.shape}")

        # Проверка совпадения размеров перед вычислением MSE
        ...
        all_targets_tensor = torch.tensor(all_targets, dtype=torch.float32).view(-1, 1).to(DEVICE)

        # Вычисляем MSE
        mse = nn.MSELoss()(all_predictions_tensor, all_targets_tensor)
        return mse.item()

def train_regression_model_batch(model, combined_features_files, y_train, batch_size=1024, epochs=15, learning_rate=0.01):
    ...
    for epoch in range(epochs):
        ...
        for i, filename in enumerate(combined_features_files):
            ...
            logging.info(f"Processing file: {filename} with shape {X_batch.shape}")
            ...
            if len(y_batch) != X_batch.shape[0]:
                logging.error(f"Skipping batch due to size mismatch: expected {X_batch.shape[0]}, got {len(y_batch)}")
                continue

            # Преобразование в тензоры
            X_batch_tensor = torch.tensor(X_batch, dtype=torch.float32).to(DEVICE)
            y_batch_tensor = torch.tensor(y_batch.values, dtype=torch.float32).view(-1, 1).to(DEVICE)
            ...
            # Очищаем ненужные данные из памяти
...
def combine_text_and_time_features_batch(X_text, X_time, batch_size=500):
    ...
    for i in range(num_batches):
        X_text_batch = X_text[i * batch_size: (i + 1) * batch_size]
        X_time_batch = X_time[i * batch_size: (i + 1) * batch_size]
        yield np.hstack((X_text_batch, X_time_batch))
